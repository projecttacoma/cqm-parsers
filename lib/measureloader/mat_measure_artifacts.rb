module Measures
  class MATMeasureData
    attr_accessor :hqmf_xml, :cql_library_artifacts, :human_readable, :components

    class CqlLibraryData
      attr_accessor :id, :version, :cql, :elm, :elm_annotations
      def initialize(id, version, cql, elm, elm_annotation)
        @id = id
        @version = version
        @cql = cql
        @elm = elm
        @elm_annotation = elm_annotation
      end
    end

    def initialize(hqmf_xml, cql_library_artifacts, human_readable)
      @hqmf_xml = hqmf_xml
      @cql_library_artifacts = cql_library_artifacts
      @human_readable = human_readable

      if @hqmf_xml.nil? || @human_readable.nil? || @cql_library_artifacts.nil? || @cql_library_artifacts.length == 0
    ...
    def self.make_measure_artifacts(measure_files)
      ...
      if !(measure_files[:elm_annotations].length == library_count && measure_files[:elms].length == library_count)
        raise MeasureLoadingException.new("Each library must have a cql file, an eml file, and an elm annotation file.")
      end

      cql_library_artifacts = []
      (0..library_count - 1).each do |i|
        elm_annotation = measure_files[:elm_annotations][i]
        elm = measure_files[:elms][i]
        cql = measure_files[:cqls][i]

        id, version = get_library_version(cql, elm, elm_annotation)
        cql_library_artifacts << CqlLibraryData.new(id, version, cql, elm, elm_annotation)
      end

      return new(measure_files[:hqmf_xml], cql_library_artifacts, measure_files[:human_readable])
    end

    private
    def self.parse_measure_files(folder)
      measure_files = { :cqls => [], :elms => [], :elm_annotations => [] }
      ...
      folder[:files].each do |file|
        ...
        when '.xml'
          ...
          if doc.root.name == 'QualityMeasureDocument'
            ...
            measure_files[:elm_annotations] << doc
          end
        end
      end
