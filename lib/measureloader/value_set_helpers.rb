module Measures
  class ValueSetHelpers
    ...
    def self.load_value_sets_and_process(cql_libraries, value_set_loader, measure_id=nil)
      elms = cql_libraries.map{ |cql_library| cql_library.elm }
      elm_value_sets = []
      elms.each do | elm |
    ...
    private
    def self.make_fake_valuesets_from_single_code_references(elms)
      ...
      elms.each do | elm |
        ...
        (elm.dig('library','codes','def') || []).each do |code_reference|
          ...
          concept = CQM::Concept.new(
            code: code_reference['id'],
    ...
    def self.set_data_criteria_code_list_ids(hqmf_model_hash, value_sets_from_single_code_references)
        ...
        if data_criteria[:inline_code_list] && !data_criteria[:code_list_id]
          ...
          data_criteria[:inline_code_list].each do |code_system, code_list|
            ...
            value_sets_from_single_code_references.each do |value_set|
              ...
              raise StandardError "wtf" unless value_set.concepts.length == 1
              concept = value_set.concepts[0]
              if concept.code_system_name.starts_with?(code_system.to_s) && code_list.include?(concept.code)
