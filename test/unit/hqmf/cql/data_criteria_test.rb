...
class DataCriteriaTest <  Minitest::Test
  ...
  def test_mat_5_6_export_communication_performed_datatype
    filename = File.join(HQMF_CQL_ROOT, 'QDM5_4_v5_6_eMeasure.xml')
    document = HQMF2CQL::Document.new(File.open(filename).read).to_model

    # Grab the Communication from the document
    data_criteria = document.all_data_criteria.find_all { |dc| dc.title == 'Communication, Performed: Communication, Performed'}
    assert_equal({'SNOMED-CT'=>['445333001']}, data_criteria[0].inline_code_list)
  end
