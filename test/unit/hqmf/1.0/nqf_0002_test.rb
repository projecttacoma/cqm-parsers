...
module HQMF1
  ...
  class NQF0002Test < Minitest::Test
    ...
    def test_to_json_0002
      ...
      _(all_criteria.length).must_equal 11
      ...
      _(population_criteria.size).must_equal 2
      ...
      _(population_criteria[0][:comparison][:restrictions].size).must_equal 1
      ...
      _(population_criteria[1][:comparison][:restrictions].size).must_equal 1

      _(population_criteria[0][:comparison][:restrictions][0][:type]).must_equal "EAS"
      ...
      _(population_criteria.size).must_equal 2
      refute_nil population_criteria[0][:comparison]
      _(population_criteria[0][:conjunction]).must_equal "AND"
      assert_nil(population_criteria[0][:preconditions])
      assert_nil(population_criteria[0][:restrictions])
      _(population_criteria[0][:comparison][:data_criteria_id]).must_equal "E929E9CB-E788-47C3-A467-4AB732D0606C"
      _(population_criteria[0][:comparison][:title]).must_equal "Patient Characteristic: birth date"
      _(population_criteria[0][:comparison][:restrictions].size).must_equal 1
      _(population_criteria[0][:comparison][:restrictions][0][:type]).must_equal "SBS"
      assert !population_criteria[0][:comparison][:restrictions][0][:negation]
      _(population_criteria[0][:comparison][:restrictions][0][:target_id]).must_equal "D578142D-F78F-4BF4-8194-82015DE21A7F"
      _(population_criteria[0][:comparison][:restrictions][0][:range][:low][:value]).must_equal "2"
      _(population_criteria[0][:comparison][:restrictions][0][:range][:low][:unit]).must_equal "a"
      _(population_criteria[0][:comparison][:restrictions][0][:range][:low][:inclusive?]).must_equal true

      refute_nil population_criteria[1][:comparison]
      _(population_criteria[1][:conjunction]).must_equal "AND"
      assert_nil(population_criteria[1][:preconditions])
      assert_nil(population_criteria[1][:restrictions])
      _(population_criteria[1][:comparison][:data_criteria_id]).must_equal "E929E9CB-E788-47C3-A467-4AB732D0606C"
      _(population_criteria[1][:comparison][:title]).must_equal "Patient Characteristic: birth date"
      _(population_criteria[1][:comparison][:restrictions].size).must_equal 1
      _(population_criteria[1][:comparison][:restrictions][0][:type]).must_equal "SBS"
      _(population_criteria[1][:comparison][:restrictions][0][:target_id]).must_equal "D578142D-F78F-4BF4-8194-82015DE21A7F"
      _(population_criteria[1][:comparison][:restrictions][0][:range][:high][:value]).must_equal "17"
      _(population_criteria[1][:comparison][:restrictions][0][:range][:high][:unit]).must_equal "a"
      _(population_criteria[1][:comparison][:restrictions][0][:range][:high][:inclusive?]).must_equal true

      population_criteria = logic["DENOM"][:preconditions]

      _(population_criteria.size).must_equal 4

      _(population_criteria[0][:conjunction]).must_equal "AND"
      _(population_criteria[0][:comparison][:data_criteria_id]).must_equal "CA71325F-084A-4F5A-9214-229D925F836D"
      _(population_criteria[0][:comparison][:title]).must_equal "Encounter: Encounter ambulatory including pediatrics"
      _(population_criteria[0][:comparison][:restrictions].size).must_equal 1
      _(population_criteria[0][:comparison][:restrictions][0][:type]).must_equal "DURING"
      _(population_criteria[0][:comparison][:restrictions][0][:target_id]).must_equal "D578142D-F78F-4BF4-8194-82015DE21A7F"

      assert !population_criteria[2][:negation]
      _(population_criteria[2][:preconditions][0][:conjunction]).must_equal "OR"
      _(population_criteria[2][:preconditions][0][:comparison][:restrictions].size).must_equal 1
      _(population_criteria[2][:preconditions][0][:comparison][:restrictions][0][:type]).must_equal "DURING"
      _(population_criteria[2][:preconditions][0][:comparison][:restrictions][0][:target_id]).must_equal "D578142D-F78F-4BF4-8194-82015DE21A7F"
    ...
    def check_data_criteria(all_criteria, key, values)
      ...
      _(data_criteria[:id]).must_equal values[:id]
      _(data_criteria[:title]).must_equal values[:title]
      _(data_criteria[:code_list_id]).must_equal values[:code_list_id]
    end
  end
end
